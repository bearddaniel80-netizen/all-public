...
@register_source(name="transformers")
class FunctionSource(IdentifierSource):
    ...
    def query(self):
        ...
        return [ item for item in PRINTABLE if item["func_type"] == FuncType.TRANSFORM]

    def show(self):
        from ....link import fn_call
        from ....link.registry import PRINTABLE, FuncType
        data = [ item for item in PRINTABLE if item["func_type"] == FuncType.TRANSFORM]
        pipeline = Pipeline([
            ShowStage(data),
        ])
