...
@register_source(name="scalars")
class ScalarSource(IdentifierSource):
    ...
    def query(self):
        ...
        return [ item for item in PRINTABLE if item["func_type"] == FuncType.SCALAR]

    def show(self):
        from ....link import scalars
        from ....link.registry import PRINTABLE, FuncType
        data = [ item for item in PRINTABLE if item["func_type"] == FuncType.SCALAR]
        pipeline = Pipeline([
            ShowStage(data),
        ])
